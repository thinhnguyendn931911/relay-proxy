...
export default function OverviewCards({ stats }) {
  return (
    <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5 sm:gap-4">
      ...
      <Card className="flex min-w-0 flex-col gap-1 px-4 py-3">
        <span className="text-text-muted text-sm uppercase font-semibold">Tokens Saved</span>
        <span className="truncate text-2xl font-bold text-info">{fmt(stats.rtkSession?.tokensSaved)}</span>
      </Card>
      <Card className="flex min-w-0 flex-col gap-1 px-4 py-3">
