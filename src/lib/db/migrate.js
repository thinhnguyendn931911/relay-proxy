...
function isPostgres(adapter) {
  return adapter?.driver === "postgres";
}
...
async function syncSchemaFromTables(adapter) {
  const compareCaseInsensitive = isPostgres(adapter);
  for (const [tableName, def] of Object.entries(TABLES)) {
    ...
    const existingNames = new Set(existing.map((r) => compareCaseInsensitive ? String(r.name).toLowerCase() : r.name));
    for (const [colName, colDef] of Object.entries(def.columns)) {
      const lookupName = compareCaseInsensitive ? colName.toLowerCase() : colName;
