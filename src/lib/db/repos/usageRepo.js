...
export async function saveRequestUsage(entry) {
  try {
    ...
    await db.transaction(async () => {
      ...
      // SaaS: UPSERT monthly usage counter per user
      if (entry.userId) {
        const periodKey = entry.timestamp.slice(0, 7).replace("-", "");
        await db.run(
          `INSERT INTO saas_usage_periods(user_id, period_yyyymm, prompt_tokens, completion_tokens, requests, updated_at)
           ...
           DO UPDATE SET
             prompt_tokens = saas_usage_periods.prompt_tokens + excluded.prompt_tokens,
             completion_tokens = saas_usage_periods.completion_tokens + excluded.completion_tokens,
             requests = saas_usage_periods.requests + 1,
             updated_at = NOW()`,
          [entry.userId, periodKey, promptTokens, completionTokens]
        );
      }
    });
